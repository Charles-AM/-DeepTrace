...
| 🔴 **G7** — model-dependent analyses were computed on **L1 (leaky) checkpoints** | `results/analysis/late_fusion/` and `cka/` reload `best.pt` from the pre-protocol-fix `c40-run` output. ESSENCE §12 places late fusion in the **main paper**; CKA is supplementary. | **permanip: CLOSED** — recomputed from L2 prediction dumps (`results/analysis/permanip_l2/`), no GPU. **late_fusion**: needs `baseline_spatial` + `frequency_only`, which exist at L2 only for **c23** (c40 never trained `frequency_only`); ~15 min GPU. **cka**: ~10 min GPU. **spectra is NOT affected** — `src/spectra.py` takes `--manifest` and loads no checkpoint; it describes the data, not a model. Its known limitation is frame-level pseudoreplication (already recorded in ESSENCE §8), and re-running it on the L2 manifest is CPU-only. |
