...
| 🔴 **G7** — mechanistic analyses were computed on **L1 (leaky) checkpoints** and never redone | `results/analysis/late_fusion/`, `cka/`, `spectra/` all originate from the pre-protocol-fix `c40-run` output. ESSENCE §12 places late fusion in the **main paper**. | permanip: **CLOSED** — recomputed from L2 prediction dumps, see `results/analysis/permanip_l2/`. late_fusion: needs `baseline_spatial` + `frequency_only`, which exist at L2 only for **c23** (c40 never trained `frequency_only`); ~15 min GPU. cka/spectra: supplementary, same treatment. |
